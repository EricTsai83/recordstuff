...
**001 初始實作與 002 檔案 log 已完成。** 003 已有部分實測結果，剩餘完整驗收待 007 完成後補齊。品質、長時間同步、權限、Windows 與發行驗收尚未全部完成。

**下一個執行項目是 007 錄製品質與可調設定**，目前尚未開始。
...
| 3 | 007 | [錄製品質與可調設定](007-recording-quality-settings.md) | 待執行 | 2026-09-12 | **下一個。** 由 Roadmap 第 7 項提前；先 A 量測調校，再 B Tray 品質選單與保存 |
| 4 | 003 | [第一次有聲音的真實錄製](003-first-real-recording.md) | 進行中 | 2026-09-12 | 基本有聲有影已確認；待用 007 最終設定補 §17 第 1、2、3、5、7、8 題 |
| 5 | 004 | [乾淨 TCC 下的權限流程測試](004-permission-flow-clean-tcc.md) | 待執行 | 2026-09-12 | 回答 §17 第 6 題；驅動 §11 的最後修正 |
| 6 | 006 | [RecordStuff.app 開發包與簽章](006-dev-app-bundle-and-signing.md) | 待執行 | 2026-09-12 | 里程碑 3 的前半；完成後第一版可發布 |
| 平行 | 005 | [Windows 環境與驗收](005-windows-environment.md) | 待執行 | 2026-09-12 | 環境建置可隨時做；錄製驗收要等 007 完成。只有 Mac，需先建 VM。006 的 Windows 部分依賴它 |
