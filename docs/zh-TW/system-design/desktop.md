...
| idle | 圓環／空白 | 待命或位置不可用；有 lastSavedPath 才能顯示最後錄影；儲存位置 | 全部可調 |
| starting | 沙漏／空白 | 提醒完成系統提示 | 語言與外觀；About 連結仍可用 |
| countdown | 碼錶／空白 | 「n 秒後開始錄製」（不可點）；「取消倒數」（已註冊快捷鍵時 tooltip 顯示組合鍵）；tooltip 說明按一下即可取消 | 語言與外觀；About 連結仍可用 |
| recording | 實心圓點／`REC` | 可停止（已註冊快捷鍵時 tooltip 顯示組合鍵）；儲存位置變灰 | 語言與外觀；About 連結仍可用 |
| stopping | 沙漏／空白 | 儲存中 | 語言與外觀；About 連結仍可用 |

每個狀態都有「設定」、「顯示 log」與「結束」，而且「設定」永遠可點：哪些偏好被鎖定由面板自己說明。每個狀態都有自己的 16 pt template（plan 040）：starting 與儲存中改用沙漏，取代原本的圓環加 `…` 標題（先試過圓環內三個小點，在選單列尺寸下太淡而未採用）；倒數顯示沒有標題的碼錶，因為被錄製螢幕的角落已顯示數字。只有 `REC` 會改變選單列項目寬度。未讀失敗的警示標記仍只取代 idle 圓環。已開啟的選單不會跟著狀態變化：在 macOS 26 與 Electron 44.3 上，`popUpContextMenu` 開啟的選單在開著時會保持原本的項目，修改項目或呼叫 `closeContextMenu()` 都沒有作用（2026-09-27 以獨立 probe 驗證），而 main 的 timer 仍照常執行。重建選單本身幾乎沒有成本，是平台不允許；改用 `setContextMenu` 則會讓左鍵也打開選單。因此每個選項都在點擊當下依目前狀態解析：倒數期間開啟的選單中的「取消倒數」，若在擷取開始後才被點選，會停止錄影並存檔。macOS 使用 template PNG／@2x，且只有 macOS 顯示圖示旁 title。Windows 沒有 title，因此 ICO 必須獨自表達狀態（plan 034）。ICO 沒有 template 模式，所以每個狀態都放在 App 圖示的深色圓角底座上，外加一圈灰色邊緣，讓同一張圖在淺色、深色與強調色工作列上都看得清楚。idle 是中心留空的白色圓環；錄影在中心加上紅色實心圓，與 idle 的差異同時在形狀與灰階，不只靠色相；警示在圓環內放琥珀色「!」；busy 與倒數沿用沙漏與碼錶，碼錶指針與外框之間留有空隙，避免看起來像電源符號。每個 ICO 含 16、20、24、32、48 px，並對齊像素格，對應通知區域 100／125／150／200% 縮放。沒有底座的候選需要深色外框，在淺色工作列上會變成細細的雙線圓環，因此未採用。這些圖示尚未在原生 Windows 工作列上觀察（[035](../../../plans/035-guided-native-acceptance.zh-TW.md) N17）。錄整個螢幕時 `REC` 與碼錶可能出現在影片，這是目前接受的呈現。

### 倒數 overlay

來源：[countdown-overlay.ts](../../../src/main/countdown-overlay.ts)、[renderer/countdown.ts](../../../src/renderer/countdown.ts)、[preload/countdown.ts](../../../src/preload/countdown.ts)、[shared/countdown.ts](../../../src/shared/countdown.ts)。

倒數期間（見[錄製設計](recording.md#倒數)），一個 88 × 88 pt 的透明視窗位於被錄製螢幕工作區右緣 16 pt、頂端下方 12 pt，只畫數字：沒有背景、方框、邊框、圓環或模糊。它放在 DisplayMedia 為這個 session 解析出的螢幕上；不知道是哪個螢幕時放在主螢幕並寫 log。視窗在準備階段建立，倒數結束、取消或失敗時銷毀，錄影之間不會殘留；App 在每次回到穩定狀態時也會再銷毀一次作為保險。它沒有邊框、固定大小與位置、沒有陰影、不出現在工作列、不可取得焦點，在 macOS 上以 non-activating panel 不啟動 App 地顯示，忽略滑鼠，位於 `screen-saver` 層級並出現在每個 Space（包括全螢幕）。它從不啟動 RecordStuff，也不奪取鍵盤焦點。頁面為本地、sandbox、context isolation，使用設定頁的 CSP；preload 只提供一個數值訂閱，main 傳送數字或 `null`，頁面無法回傳。

數字為 SF Pro Rounded semibold 56 pt、等寬數字、置中，白色 28% 不透明度，帶 1 px、10% 黑色外框與三層柔和陰影；在沒有背景的情況下，靠自身外框與陰影在白色與深色內容上都保持可讀。維護者比較過 80、55、40 與 28% 的草稿，選了最透明的一版，接受它在明亮照片上會變淡。淡入 120 ms、數字間交叉淡化 150 ms、淡出 120 ms 後才銷毀視窗；`prefers-reduced-motion` 取消淡化，`prefers-reduced-transparency` 讓數字不透明並加強外框，仍然沒有背景。不套用 content protection：`getDisplayMedia` 無法排除個別視窗，所以靠時間上的分離保護。無法取得焦點的視窗 VoiceOver 不一定會朗讀；實際表現只記錄，不宣稱支援。
...
已提交設定由主程序持有，面板只暫存尚未完成的選擇：畫出收到的 view，回傳群組 id 與選項 id，不回傳 action。主程序只接受設定視窗自身 main frame 的請求，依當下重新產生的模型解析這組 id，然後才呼叫與 tray 相同的 action handler，而該 handler 在保存前會再檢查一次錄製狀態。啟動錄製、倒數中、錄製中與存檔中，除了語言與外觀以外的設定在面板與該邊界都會鎖定。保存依請求順序序列化，第二個變更是排隊而不是被回報為失敗。保存期間顯示 main 已提交的值；儲存狀態不佔版面，並以列的 aria-busy 標示；較新的操作意圖獨立保留於佇列，不被舊回覆覆蓋，全部請求完成後才解除其他群組的鎖定，最新選擇未生效時顯示行內訊息；被 OS 拒絕註冊的快捷鍵會保留選取並加上「目前無效」的註解。群組也可以帶 `actions`：渲染在該控制項下方的按鈕，用於同一張卡片內可能凌駕該偏好的系統面板。actions 群組的選項、以及該 `actions` 清單中的選項，都沒有可比對的已提交值，因此由其 handler 以布林值回報自身結果並直接採用；其餘選項仍以「要求的值是否成為已提交值」判定，所以一個什麼都不回報的 handler 永遠無法把未保存的變更變成成功。保存期間正在操作的控制項維持可用、其餘暫時停用，因為停用中的元素無法保有鍵盤焦點。設定 preload 僅提供讀取、選取、擷取與變更訂閱；面板在第一份 view 之前唯一可能需要的字串（首次讀取失敗）依主程序寫在頁面 URL 的語言在地化。

「錄影」分頁包含螢幕選擇、倒數（以分段控制選擇關閉、3 秒、5 秒或 10 秒，並說明數字會顯示在被錄製螢幕的右上角，按一下選單列圖示或按快捷鍵即可取消）、影像品質、解析度上限與幀率，並說明編碼品質與像素尺寸的差異。「一般」包含快捷鍵、語言與更新控制。分頁支援方向鍵、Home 和 End，變更設定後保留目前分頁。更新操作使用按鈕，其完成狀態與偏好儲存分開處理。

設定視窗預設 560×680、最小 380×360，原生標題與分段分頁固定，上方以外的置中清單獨立捲動。Main 以選填的 `control`、`section`、`noteKind` 宣告呈現方式：通知與啟動更新使用原生 checkbox 開關；倒數、品質與語言使用原生 radio 分段；其他偏好保留原生選單。省略欄位時預設選單與靜態說明。連續同 section 的列共用分組清單；更新列共用標題，錄影鎖定以 section 註腳說明。明暗模式共用 accent／focus token，強制色彩回到原生控制項。
...
Tray 的儲存位置項目（[output-folder.ts](../../../src/main/output-folder.ts)，plan 033）遇到既有資料夾時照舊以 Finder 開啟。預設的 `Movies/RecordStuff` 要到開始錄影才會建立，所以開啟時也會建立它：只建這一層、使用非遞迴的 `mkdir`，而且上層必須存在並且是資料夾。不存在的自訂資料夾一律不重建：它可能在未連接的磁碟上，重建同一路徑會讓之後的錄影寫到系統磁碟。以下情況都會顯示一則在地化警告：路徑是檔案、預設資料夾的上層不存在、建立被拒、路徑無法讀取，以及 Finder 開啟失敗（`shell.openPath` 回傳錯誤字串或 reject）。警告列出完整路徑，有錯誤時附上詳細資訊，並提供「更改儲存位置」與「取消」。更改沿用上方的選資料夾流程與它的鎖；取消什麼都不改。RecordStuff 本身被拒絕存取某個資料夾時，仍會請 Finder 開啟，因為 macOS 的隱私資料夾可能拒絕 App 而不拒絕 Finder；之後 Finder 也失敗，才顯示權限警告。開啟永遠不寫入 settings.json。同一時間只進行一次：重複點擊會併入進行中的那次，警告開著時只把它帶到前景。每次失敗也會連同路徑寫入 log。錄影維持自己的 `mkdir -p` 與寫入 probe。

品質、倒數與快捷鍵只能在 idle／needsPermission 修改；每個 session 保存自己的品質與倒數快照。session 啟動、倒數、錄製或存檔期間不能變更儲存位置：它的暫存檔已在那裡開啟。設定檔即使有其他平台不開放的 60 fps，effectiveQuality 只調整本次有效值，不重寫設定。
...
「失敗紀錄」位於設定任一分頁的偏好上方。每筆可獨立展開，顯示時間、原因、檔案結果、恢復指引與技術資訊。缺少系統音訊（`no_audio_track`）的指引先提到 Mac 負載過重，再提到權限，因為兩種原因在擷取端看到的都是一開始就已結束的音軌（plan 040 的負載測試）；「開啟系統設定」與「重新啟動」仍保留。清理中停用「知道了」，不提供檔案定位。部分檔案顯示前及定位前重新檢查；未知或遺失路徑只是查找線索，不保證可播放或復原。檔案／資料夾／權限操作仍遵守錄製鎖定。普通更新保留各筆展開狀態與 ID，舊按鈕不會操作新紀錄。
