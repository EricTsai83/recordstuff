...
- **沿用基準。**「改動前」的證據可以引用同一個 runner、同一個案例的最近一次執行，條件是從那之後擷取路徑沒有改變（擷取、編碼與品質程式碼、Electron 版本、產物種類），而且機器、螢幕、音訊路徑與測試素材版本都相同。記下那次執行的日期與 commit。只有上述任一項改變，或舊結果缺少比較所需的指標時，才重新錄。
