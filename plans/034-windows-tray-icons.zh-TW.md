...
- [ ] 以已提交的 `TrayIcon` 狀態（`idle`、`recording`、`warning`，以及 040（[已完成](../docs/zh-TW/verification/history-2026-09.md#plan-040-結案--2026-09-26)）新增的 `busy` 與 `countdown`）及其既有優先順序為起點，不改 `tray-model.ts` 語意。預覽、候選比較、重新產生的 ICO 與原生檢查都涵蓋每個已提交的狀態，不只上表的三種；`busy` 與 `countdown` 沿用 040 的輪廓（沙漏、碼錶），除非選定的候選需要針對 Windows 微調。視覺草稿可獨立進行。
