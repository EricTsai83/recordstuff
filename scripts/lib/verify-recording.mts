/**
 * Runs one recording through the plan 008 verification (ffprobe → measure →
 * judge) and appends the result to `plans/measurements/<date>.md` plus a
...
export const MEASUREMENTS_DIR = path.join(REPO_ROOT, "plans", "measurements");
...
export function environmentSummary(): string[] {
  ...
  const lines = [
    `- 機器：${tryExec("sysctl", ["-n", "machdep.cpu.brand_string"]) || os.cpus()[0]?.model || "未知"}，${Math.round(os.totalmem() / 1024 ** 3)} GB`,
  ...
  if (process.platform === "darwin") {
    const displays = tryExec("system_profiler", ["SPDisplaysDataType"])
      ...
      .map((line) => line.trim())
      .join("；");
    if (displays) lines.push(`- 螢幕：${displays}`);
  }
  const ffmpeg = tryExec("ffmpeg", ["-version"]).split("\n")[0] ?? "";
  lines.push(`- ffmpeg：${ffmpeg || "未安裝"}`);
  return lines;
}
...
export function appendMeasurements(
  ...
  if (!fs.existsSync(filePath)) {
    parts.push(`# 量測 ${path.basename(filePath, ".md")}`, "", "由 `pnpm verify`／`pnpm matrix` 產生（plans/008）。門檻表見計畫檔；「主觀比對」由人填。", "", ...environmentSummary(), "");
  }
