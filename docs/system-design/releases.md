...
Sequence in one workflow run: tag/source checks → code checks (`pnpm check`) → import fixed identity → `pnpm dist:mac` → mounted verification → candidate artifact → independent publish job that reverifies without private keys and creates the public release → `verify-published` job that downloads the three assets anonymously from the public release URL, checks SHA256SUMS, reruns the mounted/signature/metadata gates and compares GitHub's own asset digests (`release.mts published`) → `record` job (stable tags only) that writes the release facts into main: package.json version, the marked README download block in both languages, a bilingual verification record skeleton and, for stable tags only, the website's `website/release-manifest.json` (regenerated from the public release by `website/scripts/manifest.mts generate`; pre-release tags keep their record commit but never reach the website), committed as `github-actions[bot]` → `deploy-website` job (stable tags only) that calls the reusable `website.yml` workflow, which checks out main, validates project access and submits source with the pinned Vercel CLI (`deploy --archive=tgz --prod`). Vercel installs `website/` dependencies and runs `pnpm test && pnpm check` before publishing the site, including online manifest verification and generated feed/link checks. It is skipped with a notice when `VERCEL_TOKEN`, `VERCEL_ORG_ID` or `VERCEL_PROJECT_ID` is not configured, has `contents: read` only and never touches the release; a website failure leaves the release public and the previous site live. The build job has contents:read; the publish and record jobs have contents:write; the verification job needs neither secrets nor write access. Signing secrets are supplied only to the build signing step. The release environment permits `v*` tags, controlled by trusted repository maintainers. One concurrency group serializes release workflows without cancelling running releases. A separate website production concurrency group serializes all website entry points without cancelling active deployments. Each website job checks out the latest main after acquiring the lock, so an older queued trigger cannot restore an older checkout. The release job calls the reusable workflow explicitly because manifest commits pushed with `GITHUB_TOKEN` do not trigger push workflows.
...
Configure repository Actions secrets `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID`; the Vercel project Root Directory remains `website/`. Vercel Git deployments stay disabled to avoid duplicate deployments. A missing secret skips deployment with a notice. Website pushes and stable App releases share the same online manifest verification, website tests, Astro diagnostics, generated feed comparison and built-link checks inside the Vercel build before its output goes live. Non-website commits do not trigger standalone website deployment; pre-release tags do not call it. To retry after configuring secrets or fixing a deployment failure:

```bash
gh workflow run website.yml --ref main
```

This workflow never builds, signs or publishes the App. After deployment, verify the public feed and actual App transport before treating update delivery as accepted.
