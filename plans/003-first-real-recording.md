...
用 `pnpm start` 在這台 Mac 錄出第一個有畫面、有聲音、長度正確、QuickTime 雙擊能播的 MP4。這是 001 里程碑 1 的核心交付；使用者已確認停止後有聲有影；目前尚無完整時長、播放器、同步與效能紀錄。使用 007 的設定與 008 的工具補齊驗收，不必重查已排除的缺音訊權限原因。步驟 5、6、9 的量測改跑 `pnpm matrix -- long` 與 `pnpm verify`，門檻以 008 的表為準。
