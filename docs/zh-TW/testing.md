...
- **取決於檔案大小的 I/O 不必錄影量測。**要回答的是某個檔案步驟（發布、複製、sync）如何隨大小變化時，在目標磁碟區用 `pnpm bench:publication` 幾秒內量完，錄影只以一般長度驗證整條流程；不要只為了製造大檔而錄長片。回答時間類問題時，`pnpm measure:finalization` 預設只完整解碼第一段，其餘只解碼最前與最後一秒。Plan 037 的 60 分鐘錄影裡約有 49 分鐘花在 8 段 5 分鐘長片上，這類量測不到一分鐘就能回答（[歷史](verification/history-2026-09.md#更快的收尾量測--2026-09-27)）。
