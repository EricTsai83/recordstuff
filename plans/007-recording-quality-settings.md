...
調校係數（008 於 2026-09-13 以 `pnpm matrix -- all` 驗證：30 fps 三等級實測位元率為目標的 99–100%，係數維持；60 fps 實得約兩倍目標；CPU 1080p30 約 15%；詳見 008 執行紀錄第三輪）：
...
008 第一次試跑（2026-09-13，`plans/measurements/2026-09-13.md`）對本計畫的回饋，待 008 正式量測後一併回填：

- `capture:` log 的 `track size=` 曾不可信：`getSettings()` 把 1920x1080 主螢幕回報成 1920x1920，1080p 上限因此錄成 1080x606、位元率 4.5 Mbps。**已修**（008，2026-09-13）：capture host 改從實際影格量尺寸，`getSettings()` 只作退路。B2 的「取得實際尺寸」需要改讀實際影格尺寸；位元率公式目前用的是約束值（像素數被高估 78%）。
- macOS loopback 音訊 track 為單聲道；008 加了 `channelCount: { ideal: 2 }` 後成品為 stereo 但左右內容相同（dual-mono）。AAC 實得約 160–167 kbps，不論要求 192 或 256 k；兩個音訊等級在本機無法顯現差異。**2026-09-13 使用者決定移除「音訊品質」子選單**，固定一個 AAC 目標；本檔上方「使用者設定」表的音訊品質一列已成歷史。
