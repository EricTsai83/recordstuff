...
There is no persistent failed state. A failure detaches the session, clears deadlines, stops the host, returns the UI to idle, then abandons the writer and reports a partial path when bytes were written. The app stores no error of its own because it has nowhere to show one; an interrupted recording leaves `<stamp>.recording.mp4` in the output folder, which the tray opens in one click. The output folder and the log are the durable diagnostics, which is why [`log.ts`](../../src/main/log.ts) writes synchronously and why Show log is offered from every tray state.
