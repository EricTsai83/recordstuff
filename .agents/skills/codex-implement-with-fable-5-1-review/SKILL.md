---
name: codex-implement-with-fable-5-1-review
description: Implement a plan or scoped change with Codex, verify it, obtain a read-only Claude Fable 5.1 review at high reasoning, immediately surface every reviewer finding before review-driven fixes, validate and fix confirmed findings, then report every finding and final disposition in the user's language alongside a purpose-led per-file summary. Use for end-to-end implementation requests that require Codex implementation with Claude Fable 5.1 or cross-model review. Fall back to an independent Codex review when Claude is unavailable and disclose the reason.
---

# Codex Implement with Fable 5.1 Review

Codex owns implementation, final technical judgment, fixes, verification, and reporting. Claude Fable 5.1 is a read-only reviewer.
...
5. Request a read-only Claude Fable 5.1 review against the requirements, diff, and check results.
6. Immediately present every returned finding to the user before making any review-driven code change. Mark these as unvalidated reviewer findings, not Codex's final judgment.
...
8. Present Codex's acceptance or rejection decisions, then fix accepted findings and rerun affected checks. Reject style-only, speculative, or false-positive findings.
...
10. Report the result in the user's language (see Language) using the format below. Include every usable finding returned by every review pass, even when it was rejected or already fixed.
...
Use Claude Fable 5.1 for every review pass. Run non-interactively at high reasoning in plan/read-only mode:
...
- 先明確說明 Claude Fable 5.1 是否完成 review；若使用 fallback，列出規定的完整 provider label。
