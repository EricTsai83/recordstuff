...
- `capture:` log 的 `track size=` 不可信：1440p 上限於 1920x1080 來源回報 1440x1440（成品 1440x810），原尺寸也回報 1920x1920（成品 1920x1080）。B2 的「取得實際尺寸」需要改讀實際影格尺寸；位元率公式目前用的是約束值（像素數被高估 78%）。
- macOS loopback 音訊 track 為單聲道（`channels=1`），成品 1 聲道，AAC 實測位元率低於 192／256 k 目標；「音訊品質」兩級的差異要在有連續音樂的素材上重量。
