...
更新：2026-09-15。本文說明目前自簽實作、開發者操作方式，以及尚待 011 實作的 CI 身分配置。編寫文件不代表已匯出私鑰、設定 secrets 或驗證 CI。
...
| CI 匯入、解鎖與使用既有身分 | 可以 | 待 011 實作及非互動驗證 |
...
[011](../../../plans/011-github-release-automation.zh-TW.md) 的「先解決簽署身分」是將既有憑證與私鑰安全提供給乾淨 runner；不需要重新申請 Apple 憑證。下列是目標配置，尚未實作或驗證：
