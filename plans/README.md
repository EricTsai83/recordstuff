...
Every plan below is planned, not implemented. Current order: **042 → 040 → 034 → 037 → 035**.

Ordering rule: zero-risk cleanup, the prioritized history work and the data-loss guards came first and are complete, as are the maintainer-requested shortcut follow-up 044 and the last audit fix, 033; now optional or measured work, and 035 last. Keep 035 last even when later-numbered implementation or fix plans are added: schedule those before it.
