...
沒有持久化的失敗狀態。失敗會卸離 session、清除期限、停止 host、讓 UI 回到 idle，接著放棄 writer，並在確實寫入過位元組時回報部分檔案路徑。app 不保存自己的錯誤狀態，因為它沒有地方能顯示；中斷的錄影會在輸出資料夾留下 `<stamp>.recording.mp4`，選單一鍵就能打開。輸出資料夾與 log 才是耐久的診斷來源，這也是 [`log.ts`](../../../src/main/log.ts) 採同步寫入、以及每個選單狀態都提供「顯示 log」的原因。
