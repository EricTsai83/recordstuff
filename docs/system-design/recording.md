...
sequenceDiagram
    ...
    H-->>R: final chunk
    H-->>R: stopped(id)
    R->>W: Drain, sync, close, exclusive link
    W-->>R: finalPath
...
Finish drains prior writes, syncs, closes, and publishes the temporary file as `.mp4` with a hard link, trying suffixes `-2`, `-3`, … on a final-name collision. A link never replaces an existing name (EEXIST moves on to the next suffix), takes constant time and needs no free space, so saving takes the same time for a 15-second and a one-hour recording and a low-disk early stop can still save. The saved file is the same file the recording wrote, so its creation time is the recording's start. A volume that refuses the link for any other reason, such as exFAT (ENOTSUP) or some network volumes, falls back for this and every later suffix to the previous exclusive copy (`COPYFILE_EXCL`), which is synced before the temporary name is removed; that copy takes time proportional to the file and needs the file's size in free space. A failed publication reports disk_full for ENOSPC, like any other write, and output_write_failed otherwise. `COPYFILE_FICLONE` is still requested but on macOS never clones: libuv implements no clone there (`COPYFILE_FICLONE_FORCE` fails with ENOSYS in Electron 44), so before plan 037 every save, on APFS too, was a full copy. After publication the temporary name is removed on a best-effort basis; only then does Recorder emit saved with the actual destination. A cleanup failure leaves the temporary name, a second link to the saved file (or a full copy after the fallback), but does not invalidate the saved file. The log's `finalize timing` line names how long the final host handoff, the queued writes, the flush, the close, the publication (`by link` or `by copy (link <code>)`) and the cleanup took. Failure first detaches the session, clears deadlines, stops the host, and returns the UI to idle; it then abandons the writer and reports a partial path when byte accounting is nonzero. Empty files are removed on a best-effort basis. Partial files are not automatically repaired or remuxed; a playable crash sample does not guarantee recovery from every interruption.
