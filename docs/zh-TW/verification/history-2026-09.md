...
- **發布量測。** `pnpm bench:publication` 在 Electron 自己的 Node 下，用正式的 FileWriter 寫入並發布每個尺寸的檔案，不需要錄影、桌面或螢幕權限。本機 APFS 上 64 MiB 與 2 GiB 兩輪都在 0–1 ms 內 by link 發布完成，整次執行 3.8 秒。6 GB 的 exFAT 磁碟映像上，2 GiB 以複製發布花 2113 ms（link ENOTSUP），和先前兩段真實錄製的 5 分鐘 exFAT 錄影（2103 與 3052 ms）一致。在該映像上要求 3 GiB 會在寫入前被拒絕（exit 2），寫入途中送 SIGINT 會 exit 130 並移除量測自己的檔案，包括 exFAT 的 `._` 附屬檔。它的 flush 階段（2 GiB 時 76–497 ms）不代表真實錄影，因為寫入速度遠超過 5 秒一次的 fsync。
- **快速驗證。** `pnpm measure:finalization` 現在預設只完整解碼第一段已儲存的錄影，其餘只讀取串流與長度，並解碼最前與最後一秒。第一版用 `ffmpeg -f null` 解碼這兩個區間，結果一段幾乎靜止畫面的錄影因「non monotonically increasing dts to muxer」訊息被判為失敗。保留的三段 15 秒錄影中，ffprobe 完整解碼每段都有 900 個影格且沒有錯誤，但 `ffmpeg -f null` 有兩段印出這類訊息。區間現在改用 ffprobe `-read_intervals` 解碼，同一回合全部通過，第一段完整解碼（停止到可再開始 33–36 ms）。截到 60% 的測試用分段式 MP4 會回報 2.03 秒而非 3.02 秒，並有尾端解碼錯誤，所以截斷的尾端仍會失敗。
- **檢查。** `pnpm check` 通過 typecheck、61 個檔案 1009 項測試與 build；新測試在完整與截斷的分段式 MP4 上執行 `probeEdges`，並涵蓋位元組大小解析。用這些工具重做 plan 037 的門檻，估計只需 10–15 分鐘桌面時間，而不是 60 分鐘；這個估計沒有實際跑一輪驗證。

未驗證：在實體外接碟上執行發布量測，以及只發生在錄影中段的損壞；後者只有 `--verify full` 或完整解碼才能發現。磁碟映像與測試錄影都已刪除。應維護者要求，變更已提交到本地 main：工具與測試 `06a4874`，以及這次的文件提交。沒有 push 或發布。
