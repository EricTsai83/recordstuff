...
## Anonymous public-download verification — passed

The `verify-published` job (added after this release) downloaded the three assets from the public release URL without a token on a GitHub runner, checked SHA256SUMS, mounted the DMG, reran the contents/signature/version/metadata gates and matched GitHub's asset digests. [Run 35441952513](https://github.com/EricTsai83/recordstuff/actions/runs/35441952513) passed in 22 seconds. The first dispatch ([run 35441201011](https://github.com/EricTsai83/recordstuff/actions/runs/35441201011)) had failed because the job checked out the tag, whose release.mts predates the `published` mode; the job now checks out the workflow ref and derives version and commit from the tag. A local re-download earlier that day was abandoned at about 55 KB/s and is not evidence.

## Not recorded

Playback in a player by the maintainer was not recorded; it is optional. and its CI run (link, size, SHA-256), with an optional browser download checksum. No result is claimed for any of these.
