...
description: Implement a plan or scoped change with Codex, verify it, obtain a read-only Claude Fable 5.1 review at high reasoning, immediately surface every reviewer finding before review-driven fixes, validate and fix confirmed findings, then report every finding and final disposition in Traditional Chinese alongside a purpose-led per-file summary. Use for end-to-end implementation requests that require Codex implementation with Claude Fable 5.1 or cross-model review. Fall back to an independent Codex review when Claude is unavailable and disclose the reason.
---
