...
以下計畫皆已規劃、尚未實作。目前順序：**035**。

排序規則：零風險整理、優先的歷史保存工作與防止資料損失的防護已排在最前並完成，維護者要求的快捷鍵後續 044、最後一項 audit 修正 033、更快的錄影回合 042、錄影前倒數 040、Windows 系統匣圖示 034（其原生 Windows 驗收移至 035），以及需量測的收尾工作 037 也已完成；037 在量測門檻結案：儲存改成連結而非複製後，已沒有可分離出來重疊的等待。只剩 035，且 035 永遠最後。即使之後新增更大編號的實作／修復計畫，也排在 035 之前。
