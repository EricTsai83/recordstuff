...
正常退出會等待錄影與所有未完成磁碟工作，即使 tray 已 idle 也一樣。等待 13 秒後若仍有工作，英文／繁中資訊對話框會提示 RecordStuff 保持開啟，請在存檔／清理完成後重試退出。重複要求不能略過此判定；啟動中的擷取在延期後仍保留停止意圖。重啟只在退出獲准後登記，退出延期會取消該次重啟意圖。不提供強制退出控制。
