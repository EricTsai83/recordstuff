...
以下計畫皆已規劃、尚未實作。目前順序：**043 → 033 → 042 → 040 → 034 → 037 → 035**。

排序規則：零風險整理、優先的歷史保存工作與防止資料損失的防護已排在最前並完成；目前先處理維護者要求的快捷鍵修正 043，再依原順序處理 audit 修正，最後是可選或需量測的工作，035 永遠最後。即使之後新增更大編號的實作／修復計畫，也排在 035 之前。

| 計畫 | 來源 | 範圍 |
| --- | --- | --- |
| [043 — 快捷鍵依實體鍵位](043-physical-shortcut-keys.zh-TW.md) | 2026-09-26 plan 032 發現；維護者要求；Cap 比較 | 全域快捷鍵依實體鍵位註冊，讓預設 ⌘⇧1 在注音及其他數字列不輸入數字的配置下可用；非 QWERTY 拉丁配置的字母快捷鍵依 US 位置，為已接受的取捨 |
...
- 043 與 033 可獨立執行；043 是使用者可見的快捷鍵失效，排在最前面。
...
- 各計畫的 Cap 比較固定 revision `ce785e705e79652adba4b8bf752669c4093499e0`；037 與已結案的 038 固定 `26e1a6d882f311d10b5317e9e0d29babe4f6737e`；040 固定 `119edf04864b59abfc0d52f60bf77d7f33cfd2b8`；043 固定 `40f44a803f0980fb7ed530f17d12b8fed40f6b5a`；已結案的 041 固定 `b2b6ae45d4cae303107b10a9df166d91caed7702`，並閱讀 Chromium tag 152.0.7977.78。只代表已檢視範圍的靜態比較，不能外推為 Cap 已處理這些問題。
