...
- [ ] Give each launch one run id (initial form: launch time plus pid), printed in the existing `start:` line and carried in every structured record, so consumers can tell a restarted app from a rotated file and pair sessions across launches without guessing from line order. Do not prefix every human-readable line; the start line and the structured records are enough. Merged 2026-09-25 from the T3 Code/Cap architecture comparison: T3 Code stamps a per-run id on every log line.
