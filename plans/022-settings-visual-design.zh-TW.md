...
狀態：尚未開始；設計已提出，程式尚未動工。優先順序：下一個；021 已結案。建立：2026-09-21。更新：2026-09-23，整合 focus、自訂快捷鍵 UI 精修、021 錯誤辨識度，以及獨立視覺語言、官方連結區與克制的恢復操作。

## 問題與目標

設定面板是 RecordStuff 唯一的視窗。使用者看得到的其他東西只有一個選單列圖示、一則通知和一個檔案，所以整個 App 的視覺印象全押在這一個視窗上 — 而它現在看起來就像一張沒有樣式的表單。每一項偏好，包括只有開與關兩種狀態的那兩項，都畫成同一個滿版 `<select>`，包在同一個有框的卡片裡（[renderer/settings.ts](../src/renderer/settings.ts)、[settings.css](../src/renderer/settings.css)）。「一般」分頁堆了六張一模一樣、彼此沒有任何分組關係的卡片，快捷鍵、兩個更新控制項、通知與語言因此讀起來像權重相同的陌生人。其中三張卡片還永遠掛著一段說明文字。保存失敗時，紅色訊息出現在視窗最下方，離出問題的控制項很遠。焦點框的藍色分散寫在各條規則，尚未整合成一致的 App 設計 token；固定品牌色本身不是問題；而內容欄的寬度上限是 560px，卻裝在 460px 的視窗裡，這條規則從來沒生效過。

把面板做成 macOS 使用者已經認得的形狀：分組的內縮清單、該是開關的東西就用開關、真正是清單的才用彈出選單、採用一致的 App 色彩，並且讓失敗出現在它發生的地方。面板必須維持和現在一模一樣的誠實程度 — 同樣的偏好、同樣的 id、同樣的鎖定規則、同樣的保存路徑 — 而且繁體中文不能比英文更難讀。範圍是設定視窗；tray、通知與首次啟動提示不在本計畫內。

## 設計決策

### 視覺方向：獨立的 App 設計語言，帶有 macOS 風格

依使用者於 2026-09-23 補充的偏好，整體以「有獨立設計語言、帶 macOS 風格的精緻工具」為視覺方向。未來可延伸至 Windows，但本計畫不新增平台支援或驗收。延續系統字體、原生視窗框架、熟悉的控制項比例、中性色表面、細分隔線、適度圓角與 App 自有強調色；設計感來自留白節奏、對齊、文字層級及互動狀態的細緻一致。原生 popup 與鍵盤操作維持平台習慣，快捷鍵符號與按鍵外觀保持輕巧。
...
「一般」分頁底部保留一個可辨識但低調的「關於 RecordStuff」區塊，包含 App 名稱，以及「官方網站」與「GitHub 原始碼」兩個文字連結；目標沿用專案的 https://record.ericts.com 與 https://github.com/EricTsai83/recordstuff。放在可捲動內容末端，以留白或細分隔線與偏好區分，不新增分頁、不做大型品牌卡片、不佔用固定頁尾空間。窄視窗可正常換行；連結有清楚的鍵盤焦點與可讀名稱，錄影期間仍可開啟。

連結透過 main 授權的固定目的地在預設瀏覽器開啟，不讓設定視窗導向網站，也不接受 renderer 任意 URL。可新增必要的連結 action id，但既有偏好／選項 id 與保存契約不變；這是上述「不改偏好」範圍內的導航操作。雙語、開啟失敗回饋與安全邊界納入 fixture／原生驗收。
...
上圖是資訊層級示意，不是額外巢狀卡片的要求。診斷用適度暖色提示、清晰標題與留白；一般說明維持次級文字。原生選單內的失效保存項目加上「無法使用」後綴並停用該項，選單本身仍可操作。標籤、id、保存名稱與狀態後綴分開，後綴不寫入偏好。最多一個小型次要恢復按鈕；原選單就是「選其他螢幕」的入口，不再增加重複按鈕。
...
快速修正不是每個診斷的必要配件。預設沿用旁邊原控制項；僅在目前確實阻擋錄製、main 提供有效替代選擇，而且有明確單一步驟時，顯示「改用主螢幕」。它是可選的便利操作，不宣稱主螢幕就是使用者原本想錄的內容。使用與其他次要按鈕相同的尺寸及中性樣式，置於原因文字下方並對齊內容，不使用滿版強調色或獨立卡片，正常狀態不留空白佔位。歷史失敗不單獨觸發此按鈕，快捷鍵衝突沿用現有選單／更改入口；保存失敗的重試仍受上表有效性條件約束。驗收比較有／無錯誤時的密度與最小尺寸，若快捷動作造成重複或擁擠，優先保留原控制項及清楚指引，記錄取捨，不以按鈕數量作為完成標準。
...
`color-scheme: light dark` 與 `light-dark()` 保留，明暗模式仍跟隨系統。一般狀態的焦點、選取與開關使用 App 自有強調色，不要求跟隨 macOS 的使用者強調色，也不新增主題偏好。以現有藍色作為設計起點，將色彩集中成語意 token，最終色值經雙語淺／深色實際畫面確認；原生 popup 等平台繪製部分保留平台行為。文字、邊框與底色集中管理，文字配對至少 4.5:1；forced-colors 下讓系統輔助色彩優先。未來移植時保留相同語意 token，按平台調整原生控制項與操作習慣。
...
- App 強調色需提供淺／深色下可讀的配對；`forced-colors` 使用系統色與實線 outline，不能只依賴 box-shadow。確認未被容器裁切、不引發版面位移。選取、hover、focus、disabled 各有清楚區別。
...
- 淺色與深色都跟隨系統，焦點框使用一致的 App 強調色，純鍵盤操作能依可見順序走到每個控制項，⌘W 仍然關閉視窗；Escape 在捕捉時先取消編輯，其餘狀態維持關窗。
...
- [ ] 以單一 token 區塊重寫 [settings.css](../src/renderer/settings.css)：底色、邊框、兩級文字、圓角、App 自有強調色及其淺／深色配對，以及 macOS 字級階層。
...
- [ ] 實作一般分頁底部的官方網站／GitHub 原始碼區塊、main 固定目的地授權及雙語文案；驗證鍵盤操作、外部瀏覽器開啟、失敗回饋、錄影期間可用與窄視窗排版。
- [ ] 驗證快捷恢復只在符合條件時出現，視覺為次要操作；歷史提示及一般狀態不增加多餘按鈕。
...
- [ ] 依[原生 computer-use 驗收 skill](../.agents/skills/astra-acceptance-with-computer-use/SKILL.md) 使用 `pnpm start:app` 的實際 App。重建／關閉前確認沒有使用者正在錄影。驗收鍵盤單獨操作、滑鼠操作、VoiceOver、非預設系統強調色下 App 配色仍一致、Increase contrast 與可測的 forced-colors；未測項目明列。
...
- [MDN: accent-color](https://developer.mozilla.org/en-US/docs/Web/CSS/accent-color)：以 App 強調色替原生核取方塊與單選鈕上色。
