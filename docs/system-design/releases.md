...
Configure repository Actions secrets `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID`; the Vercel project Root Directory remains `website/`. Vercel Git deployments stay disabled to avoid duplicate deployments. A missing secret skips deployment with a notice. Website pushes and stable App releases share the same online manifest verification, website tests, Astro diagnostics, generated feed comparison and built-link checks before deploying the exact prebuilt output. Non-website commits do not trigger standalone website deployment; pre-release tags do not call it. To retry after configuring secrets or fixing a deployment failure:
