...
- [ ] Map no media to existing capture_start_failed when appropriate, with clear bilingual feedback; if a new code is necessary, update the shared validator, translations and every consumer together. Never emit saved or set lastSavedPath for zero output. Real write errors retain their original code.
