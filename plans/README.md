...
Next: [022 — Settings panel visual design](022-settings-visual-design.md) (not started). The panel is the only window the app has, and every preference — including the two that are simply on or off — renders as the same full-width dropdown in the same card. 022 groups the rows into sections, lets main declare a switch, a segmented control or a popup menu per group, reports a failure at the control that failed, and centralizes an app-owned accent with macOS-inspired styling and preserves an official website/source links area. It also refines focus, consolidates custom shortcut entry, and adds inline capture, key previews, local feedback and focus restoration within a refined macOS style. Following 021 manual feedback, errors also become distinct from ordinary help through visible icons, headings, reasons and recovery guidance, without requiring hover or notifications; restrained, conditional secondary recovery actions such as Use Primary display, truthful save results and focus handling must let users understand and complete recovery. No preference, id, default or lock rule changes.
