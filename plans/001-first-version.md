...
- 手動檢查表（每次發版前，兩個 OS 各跑一次）：錄 10 分鐘、在 Finder / 檔案總管雙擊用系統預設播放器播放、音畫偏移、CPU、權限拒絕與允許、需重啟、錄製中結束、更改位置到外接硬碟後拔掉、資料夾不存在、硬碟滿、深淺色選單列圖示。錄 10 分鐘、音畫偏移與 CPU 改用 `pnpm matrix -- long`（2026-09-13 起為 3 分鐘：10 分鐘已量過一次，3 與 10 分鐘漂移同為 3 ms，使用者決定回歸以 3 分鐘為準）。
