...
Every plan below is planned, not implemented. Current order: **034 → 037 → 035**.

Ordering rule: zero-risk cleanup, the prioritized history work and the data-loss guards came first and are complete, as are the maintainer-requested shortcut follow-up 044, the last audit fix, 033, the faster recording rounds, 042, and the recording countdown, 040; now optional or measured work, and 035 last. Keep 035 last even when later-numbered implementation or fix plans are added: schedule those before it.
...
- 034's Windows artwork must cover every committed tray state, including 040's (complete) `busy` and `countdown`.
