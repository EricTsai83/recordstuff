...
以下計畫皆已規劃、尚未實作。目前順序：**044 → 033 → 042 → 040 → 034 → 037 → 035**。

排序規則：零風險整理、優先的歷史保存工作與防止資料損失的防護已排在最前並完成；目前先處理維護者要求的快捷鍵後續 044，再依原順序處理 audit 修正，最後是可選或需量測的工作，035 永遠最後。即使之後新增更大編號的實作／修復計畫，也排在 035 之前。

| 計畫 | 來源 | 範圍 |
| --- | --- | --- |
| [044 — 鍵盤配置快捷鍵檢查](044-shortcut-layout-check.zh-TW.md) | 043 後續；維護者要求 | 一個指令自動選取已啟用、數字列不輸入數字的配置，以數字列與數字鍵盤 key code 檢查建置後 App 的數字快捷鍵，最後還原輸入法；負向 drill 證明它能抓到 043 的錯誤 |
...
- 044 與 033 可獨立執行；044 建立在已結案的 043 之上。
