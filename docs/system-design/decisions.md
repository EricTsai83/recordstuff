...
| Tag is the release; verification precedes tagging | One maintainer and one Mac cannot support nightly channels, and an in-pipeline manual acceptance gate only repeated checks already done during development | CI proves build, signature, layout and byte identity but not capture; a bad release is fixed by a new version, never by overwriting |
